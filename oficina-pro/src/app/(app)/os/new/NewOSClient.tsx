...
export default function NewOSClient({ workshopId, stock }: Props) {
  ...
  async function addExternal() {
    ...
    await supabase.from('notifications').insert({
      ...
      message: `Peça externa "${extName}" utilizada em OS — considere adicionar ao estoque.`,
    })
  ...
  async function saveAndSign() {
    ...
    const { data: os, error: osErr } = await supabase.from('orders').insert({
      ...
      client, vehicle, items, labor, total, obs, signed: false, delivered: false,
    }).select().single()

    if (osErr || !os) { setError('Erro ao salvar OS.'); setLoading(false); return }
    setSavedOSId(os.id)
    setLoading(false)
    setSigning(true)
  ...
  async function confirmSig() {
    const cv = canvasRef.current
    if (!cv || !savedOSId) return
    const sig = cv.toDataURL()
    await supabase.from('orders').update({ signed: true, sig_auth: sig }).eq('id', savedOSId)
  ...
  if (signing) return (
    <div className="max-w-lg mx-auto">
      ...
      <p className="text-sm text-[#9090b0] mb-6">Eu, <strong>{client.name}</strong>, autorizo a execução dos serviços, no valor total de <strong>R$ {total}</strong>.</p>
  ...
  return (
    <div className="max-w-xl mx-auto">
      <h1 className="font-heading text-2xl font-bold mb-6">Nova Ordem de Serviço</h1>
      ...
      {step === 1 && (
        <div className="card space-y-3">
          <h2 className="font-heading text-base font-bold mb-2">Dados do Veículo</h2>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input className="input-base" placeholder={f.placeholder}
                  value={(vehicle as any)[f.key]} onChange={e => setVehicle({ ...vehicle, [f.key]: f.key === 'plate' ? e.target.value.toUpperCase() : e.target.value })} />
              </div>
            ))}
          </div>
          <div>
      ...
      {step === 2 && (
        <div className="card space-y-4">
          <h2 className="font-heading text-base font-bold">Peças e Materiais</h2>
          ...
          <div>
            <p className="text-xs font-semibold text-[#9090b0] uppercase tracking-wider mb-2">🔗 Peça externa</p>
            <div className="flex gap-2">
              <input className="input-base flex-1" placeholder="Nome da peça" value={extName} onChange={e => setExtName(e.target.value)} />
            ...
            {items.map((it, i) => (
              <div key={i} className="flex items-center gap-3 bg-[#18181f] rounded-xl px-4 py-2.5">
                <div className="flex-1">
                  ...
                  <span className="text-xs text-[#505068] ml-2">×{it.qty}</span>
                  <span className={`ml-2 text-xs ${it.from_stock ? 'badge-green' : 'badge-yellow'}`}>
                    {it.from_stock ? 'Estoque' : 'Externo'}
                  </span>
                </div>
                <span className="text-green-400 font-semibold text-sm">R$ {it.price}</span>
                <button onClick={() => setItems(items.filter((_, j) => j !== i))} className="text-red-400 hover:text-red-300 text-sm">🗑️</button>
              </div>
            ))}
            {!items.length && <p className="text-xs text-[#505068]">Nenhuma peça adicionada</p>}
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="card space-y-4">
          <h2 className="font-heading text-base font-bold">Orçamento Final</h2>
          <div className="bg-[#18181f] rounded-xl p-4 space-y-3">
            <div className="flex justify-between text-sm"><span className="text-[#9090b0]">Peças / materiais</span><span>R$ {partsTotal}</span></div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#9090b0]">Mão de obra</span>
          ...
          <div>
            <label className="block text-xs font-semibold text-[#9090b0] uppercase tracking-wider mb-1">Observações técnicas</label>
            <textarea className="input-base" rows={2} placeholder="Notas da avaliação..." value={obs} onChange={e => setObs(e.target.value)} />
          </div>
          <div className="bg-[#ff6b2b]/10 border border-[#ff6b2b]/25 rounded-xl p-3 text-sm text-[#ff6b2b]">
            ✏️ Após salvar, o cliente irá assinar digitalmente para autorizar o serviço.
          </div>
          {error && <p className="text-red-400 text-sm">{error}</p>}
        </div>
      )}
      ...
      <div className="flex gap-3 justify-between mt-5">
        {step > 0
          ? <button onClick={() => setStep(step - 1)} className="btn-ghost">← Voltar</button>
        ...
        {step < 3
          ? (
            ...
            }} className="btn-primary">Próximo →</button>
          ) : (
