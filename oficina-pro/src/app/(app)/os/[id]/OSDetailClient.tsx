...
const STATUS_LABELS: Record<string, string> = { open: 'Aberta', progress: 'Em andamento', done: 'Concluida' }
...
export default function OSDetailClient({ order, workshop, role }: { order: any; workshop: any; role: string }) {
  ...
  function initCanvas() {
    setTimeout(() => {
      ...
      const pos = (e: any) => {
        const r = cv.getBoundingClientRect(); const s = e.touches ? e.touches[0] : e
  ...
  function printOS() {
    ...
    const payLabel = o.payment_method ? PAY_LABELS[o.payment_method] || o.payment_method : 'Nao informado'
    const payIcon  = o.payment_method ? PAY_ICONS[o.payment_method] || '' : ''
...
.pay-box{background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;padding:12px 16px;display:flex;align-items:center;gap:12px;margin-bottom:20px}
.pay-label{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#0369a1;margin-bottom:2px}
.pay-val{font-size:16px;font-weight:700;color:#0369a1}
.total-box{background:#f8f8f8;border-radius:8px;padding:14px;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}
...
<div class="grid">
  ...
  <div class="block"><div class="block-label">Veiculo</div><div class="block-val">${o.vehicle?.model}</div><div class="block-sub">Placa: ${o.vehicle?.plate}${o.vehicle?.color ? ` | Cor: ${o.vehicle.color}` : ''}${o.vehicle?.km ? ` | Km: ${o.vehicle.km}` : ''}</div></div>
</div>
...
<tr><td><strong>Mao de obra</strong></td><td>-</td><td>R$ ${o.labor}</td><td>-</td></tr>
</tbody></table>
<div class="total-box"><span class="total-label">Valor Total</span><span class="total-val">R$ ${o.total}</span></div>
<div class="pay-box">
  <span style="font-size:24px">${payIcon}</span>
  <div><div class="pay-label">Forma de Pagamento</div><div class="pay-val">${payLabel}</div></div>
</div>
${o.obs ? `<p style="font-size:12px;color:#888;margin-bottom:16px"><strong>Obs:</strong> ${o.obs}</p>` : ''}
<h2>Assinatura de Autorizacao</h2>
<p style="font-size:12px;color:#888;margin-bottom:8px">Autorizo a execucao dos servicos pelo valor de <strong>R$ ${o.total}</strong> via ${payLabel}.</p>
...
${o.rating ? `<h2 style="margin-top:20px">Avaliacao de Entrega</h2><p style="font-size:13px">Pecas: ${'★'.repeat(o.rating.parts)} | Limpeza: ${'★'.repeat(o.rating.cleaning)} | Resolvido: ${'★'.repeat(o.rating.resolved)}</p>${o.sig_del ? `<img src="${o.sig_del}" style="height:80px;border:1px dashed #ccc;border-radius:8px;display:block;margin-top:8px">` : ''}` : ''}
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {isOwner && <button onClick={printOS} className="btn-ghost text-sm">Imprimir PDF</button>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="card-sm">
          <p className="text-xs font-semibold text-[#9090b0] uppercase tracking-wider mb-2">Veiculo</p>
      ...
      <div className="card-sm mb-4">
        <p className="text-xs font-semibold text-[#9090b0] uppercase tracking-wider mb-2">Problema</p>
      ...
      {/* Forma de pagamento */}
      {o.payment_method && (
        <div className="card-sm mb-4 flex items-center gap-3">
          <span className="text-2xl">{PAY_ICONS[o.payment_method] || '💰'}</span>
          <div>
            <p className="text-xs font-semibold text-[#9090b0] uppercase tracking-wider">Forma de Pagamento</p>
            <p className="font-semibold text-sm mt-0.5">{PAY_LABELS[o.payment_method] || o.payment_method}</p>
          </div>
        </div>
      )}

      <div className="card mb-4">
        <p className="text-xs font-semibold text-[#9090b0] uppercase tracking-wider mb-3">Pecas e Servicos</p>
        <table className="w-full">
          <thead><tr>
            <th className="table-th">Item</th><th className="table-th">Qtd</th>
            <th className="table-th">Valor</th><th className="table-th">Origem</th>
          </tr></thead>
          <tbody>
            {(o.items || []).map((it: any, i: number) => (
              <tr key={i} className="hover:bg-[#18181f]">
                ...
                <td className="table-td">
                  <span className={it.from_stock ? 'badge-green' : 'badge-yellow'}>
                    {it.from_stock ? 'Estoque' : 'Externo'}
                  </span>
                </td>
              </tr>
            ))}
        ...
        <div className="flex justify-between text-sm mt-3 pt-3 border-t border-[#252530]">
          <span className="text-[#9090b0]">Mao de obra</span><span>R$ {o.labor}</span>
        </div>
      ...
      {o.rating && (
        <div className="card-sm mb-4">
          <p className="text-xs font-semibold text-[#9090b0] uppercase tracking-wider mb-2">Avaliacao</p>
          <div className="flex gap-4 text-sm">
            <span>Pecas: {'★'.repeat(o.rating.parts)}{'☆'.repeat(5 - o.rating.parts)}</span>
      ...
      {showDelivery && (
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4" onClick={() => setShowDelivery(false)}>
          <div className="bg-[#111118] border border-[#252530] rounded-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <h2 className="font-heading text-lg font-bold mb-4">Avaliacao de Entrega</h2>
            {[
              { label: 'Qualidade das pecas',    key: 'parts'    },
              { label: 'Limpeza do veiculo',     key: 'cleaning' },
        ...
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4">
          <div className="bg-[#111118] border border-[#252530] rounded-2xl w-full max-w-lg p-6">
            <h2 className="font-heading text-lg font-bold mb-2">
              {signing === 'auth' ? 'Assinatura de Autorizacao' : 'Assinatura de Entrega'}
            </h2>
            <p className="text-sm text-[#9090b0] mb-4">
              {signing === 'auth'
                ? `Autorizo os servicos do ${o.number} pelo valor de R$ ${o.total}.`
                : 'Confirmo que recebi o veiculo em perfeito estado.'}
            </p>
